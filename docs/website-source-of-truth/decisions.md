...
- **Grid: 7×7 at 7.5 miles (49 points, 2.5 mi apart). SUPERSEDES the study's first pick of 11×11 at 12 mi (Matt: too thorough).** Radius study, not copied from Jared/Elevated Audience's 7×7 at 6 mi (see `ll-heatmap/docs/RADIUS_STUDY.md`): our clients' priority towns sit a median 12 miles out; top-3 visibility usually ends ~2.5 miles from the pin; service-area businesses rank off-center.
...
- **Ad signal (2026-09-24, Matt):** the page fires Meta `Lead` / GA `generate_lead` ONLY when the scanned listing is green industry (decided server-side from its Google category, returned as `greenIndustry`). Other listings fire `HeatmapScanNonGreen` / `heatmap_scan_non_green` instead and are tagged `not-green-industry` in SAE. Reason: Meta's Advantage+ targeting learns from the Lead event, so non-green leads would teach it to find plumbers and roofers.
