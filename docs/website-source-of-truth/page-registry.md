...
| Core | Contact | `/contact/` | **developed** | Built 2026-06-19. Hero (booking CTA) → two-column: "Send us a message" = a **custom-styled form** (Company, Full Name, Email, Phone, "How can we help your business?") that POSTs JSON to a **GoHighLevel inbound webhook** then redirects to `/confirmation/` (inline script, `mode:'no-cors'`; webhook URL is a placeholder const awaiting the owner). All fields required + show lighter placeholder text (white on input); a **mandatory opt-in consent checkbox** (required) links to `/terms/` + `/privacy-policy/`. + "Reach us directly" (email hello@, St. Petersburg FL, **business hours Mon-Fri 9am-5pm ET**, socials, book-a-call nudge). Social icons reuse the **exact footer `.fv2-social`** markup/CSS (render identically); the "Already serious about your growth?" book-a-call nudge is a **Twilight-purple `.btn--twilight` button** (white text). `ContactPage` + `Organization` + `BreadcrumbList` schema. **PENDING OWNER: GHL inbound-webhook URL (+ optional phone).** |
