...
| Core | Contact | `/contact/` | **developed** | Built 2026-06-19. Hero (booking CTA) → two-column: "Send us a message" (GoHighLevel form slot `#contact-form`, awaiting the owner's Service Area Expert/GHL embed) + "Reach us directly" (email hello@, St. Petersburg FL location, socials, book-a-call nudge). `ContactPage` + `Organization` (contactPoint + address) + `BreadcrumbList` schema. **PENDING OWNER: GHL form embed code (+ optional phone number).** |
